<template>
  ...
  >
    <div class="w-3/4 md:w-1/3 items-start">
      <div class="flex items-center gap-5 flex-col sm:flex-row">
        <h1 class="text-2xl sm:4xl font-bold font-Roboto text-white">
          {{ props.type == "tv" ? obj.name : obj.title }}
        </h1>
        <div class="flex items-center gap-2">
          <h3 class="text-xs sm:text-base font-bold font-Roboto text-slate-300">
            {{
              props.type == "tv"
          ...
          <h3 class="text-white md:text-green-300 font-Roboto text-xs sm:text-base">
      ...
      <p
        class="font-Roboto text-base sm:text-xl italic text-slate-100 line-clamp-2 sm:line-clamp-4"
      >
        {{ obj.overview }}
      </p>
    </div>
    ...
    >
      ...
      ></span>
      <span class="relative"
        >Salvar <i class="fa-solid fa-heart group-hover:animate-bounce"></i
      ></span>
    </button>
    <button
      v-if="isFavorited"
    ...
    >
      ...
      ></span>
      <span class="relative"
        ><i class="fa-solid fa-heart-crack group-hover:animate-bounce"></i>
        Remover</span
      >
    </button>
  </div>
  <div
    v-else
    class="h-screen w-screen overflow-hidden absolute flex flex-col justify-center items-center"
  >
